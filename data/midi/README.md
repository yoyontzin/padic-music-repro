...
> **Reproducibility note.** The Mutopia project sometimes updates engravings and quantizations; the files in this bundle are the exact byte sequences used to generate the results in `results/` (see `MANIFEST.txt` for SHA-256 hashes). To download fresh copies, run `python code/get_mutopia_midis.py`.
